...
% reading hologram in BIN format
  ...
  % image resize %
      raw = imread('sample59_398.png');
      raw_rz = double(imresize(raw, [1024 1024]));
      hologram = raw_rz;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%       title('Fourier spectrum in log scale / a.u.')
      ...
      set(gca,'YDir','normal')
      colormap('gray')
%       imwrite(flipud(rot90(log(abs(spectrum3)))), 'croped_spec.jpg');
...
%     im = rec_phase_unwrapped;
%     n = size(unique(reshape(im,size(im,1)*size(im,2),size(im,3))),1);
%     rgb = ind2rgb(gray2ind(rec_phase_unwrapped,n),jet(n));
...
% Saving reconstructed amplitude as JPG image
        ...
        rz_p = imresize(p, [2056 2464]);
        imwrite (rz_p, 'Rec_am.jpg');
